...
export async function PATCH(
  ...
  try {
    ...
    const alerta = await prisma.alertaSanitaria.findUnique({
      where: { id: params.id },
    });
    ...
    const { estado, accionTomada, responsable, fechaResolucion } = await request.json();

    const updated = await prisma.alertaSanitaria.update({
      where: { id: params.id },
      data: {
        ...(estado !== undefined && { estado }),
        ...(accionTomada !== undefined && { accionTomada }),
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.alertaSanitaria.delete({
      where: { id: params.id },
    });
