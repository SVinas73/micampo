...
/**
 * Motor del feed de decisiones del día.
 * Junta CLIMA + SATÉLITE (NDVI) + SANIDAD + RIEGO + ECONOMÍA + SINERGIAS
 * cruzadas (clima×cosecha×maquinaria) en una lista priorizada de acciones.
 * Datos reales; cada item enlaza al módulo para actuar. Reutilizado por
 * /api/decisiones y por el push de alertas (lib/notify).
 */
...
const CULTIVOS_COSECHA = ["Soja", "Maíz", "Trigo", "Girasol", "Sorgo", "Cebada"];
const MAQ_NO_OPERATIVA = ["Mantenimiento", "Averiado", "Fuera de Servicio"];

export async function construirDecisiones(userId: string): Promise<{ decisiones: Decision[]; generado: string }> {
  const hace7 = Date.now() - 7 * 86400000;
  const [lotes, lluvias, precios, cosechadoras] = await Promise.all([
    ...
    prisma.maquinaria.findMany({ where: { establecimiento: { userId }, tipo: "Cosechadora" } }).catch(() => []),
  ]);
  ...
  // ---- Clima (Open-Meteo) ----
  let dias: { tmax: number; tmin: number; lluvia: number; hrMax: number; viento: number }[] = [];
  let lluviaProx3 = 0;
  try {
    ...
    if (r.ok) {
      ...
      const cur = w.current || {};
      const d = w.daily || {};
      dias = (d.time || []).map((_t: string, i: number) => ({
      ...
      lluviaProx3 = dias.slice(0, 3).reduce((s, x) => s + (x.lluvia || 0), 0);
      // Helada esta noche
      ...
      if (lluviaProx3 >= 20) {
        decisiones.push({ id: "lluvia", categoria: "Clima", titulo: "Lluvia importante en camino", detalle: `${Math.round(lluviaProx3)} mm previstos en 3 días. Posponé pulverizaciones y planificá labores.`, prioridad: "media", impacto: "Aprovechá o protegé según el cultivo", ruta: "/plan-riego", icono: "droplet", color: "#2c6bb8" });
      }
    }
  } catch { /* sin clima */ }

  // ---- SINERGIA clima × cosecha × maquinaria ----
  // Si viene lluvia, hay cultivo cosechable y la cosechadora NO está operativa,
  // hay que resolver el equipo y coordinar la cosecha ANTES de que llueva.
  const cosechables = conCultivo.filter((l) => CULTIVOS_COSECHA.includes(l.cultivo || ""));
  const cosechadoraCaida = cosechadoras.find((m) => MAQ_NO_OPERATIVA.includes(m.estado));
  if (lluviaProx3 >= 15 && cosechables.length > 0 && cosechadoraCaida) {
    const l = cosechables[0];
    decisiones.push({
      id: "sinergia-cosecha",
      categoria: "Sinergia",
      titulo: "Resolvé la cosechadora antes de la lluvia",
      detalle: `Se prevén ${Math.round(lluviaProx3)} mm en 3 días y tu cosechadora ${cosechadoraCaida.marca} ${cosechadoraCaida.modelo} está ${cosechadoraCaida.estado.toLowerCase()}. Tenés ${cosechables.length} lote(s) con ${l.cultivo} para cosechar (ej. ${l.nombre}): apurá el taller o conseguí un contratista para no perder ventana ni calidad de grano.`,
      lote: l.nombre,
      prioridad: "alta",
      impacto: "Evita pérdida de cosecha por lluvia",
      ruta: "/maquinaria",
      icono: "wrench",
      color: "#c93434",
    });
  ...
  return { decisiones, generado: new Date().toISOString() };
}
